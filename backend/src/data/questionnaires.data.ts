...
export const SECTOR_QUESTIONNAIRES: Record<SectorCategory, SectorQuestionnaire> = {
  ...
  [SectorCategory.PUBLIC_ADMIN]: {
    ...
    questions: [
      createQuestion('pub_env_1', DiagnosticCategory.ENVIRONMENTAL, 'Plans de sobriété énergétique sur bâtiments publics ?', 'boolean', 2),
      ...
      createQuestion('pub_env_3', DiagnosticCategory.ENVIRONMENTAL, 'Réemploi de mobilier/équipements (délégataires) ?', 'boolean', 2),
      ...
      createQuestion('pub_eco_3', DiagnosticCategory.ECONOMIC, 'Efficience des prestations externalisées ?', 'choice', 1, undefined, ['Élevée', 'Moyenne', 'Faible']),
      createQuestion('pub_eco_4', DiagnosticCategory.ECONOMIC, 'Open data / transparence des résultats environnementaux ?', 'boolean', 2),
      
      createQuestion('pub_social_1', DiagnosticCategory.SOCIAL, 'Emplois locaux, insertion et alternance dans les marchés ?', 'percentage', 2, '%'),
      ...
      createQuestion('pub_social_4', DiagnosticCategory.SOCIAL, 'Concertation citoyenne et qualité de service perçue ?', 'boolean', 1),
      
      createQuestion('pub_log_1', DiagnosticCategory.LOGISTICS, 'Plan de mobilité des agents (PDM€) ?', 'boolean', 2),
      createQuestion('pub_log_2', DiagnosticCategory.LOGISTICS, 'Gestion du parc auto (électrification, taux d\'usage) ?', 'boolean', 2),
      createQuestion('pub_log_3', DiagnosticCategory.LOGISTICS, 'Logistique des équipements sensibles (traçabilité) ?', 'boolean', 1),
  ...
  [SectorCategory.HOSPITALITY]: {
    ...
    questions: [
      createQuestion('hosp_env_1', DiagnosticCategory.ENVIRONMENTAL, 'Consommations énergétiques par chambre/couvert (kWh/unité) ?', 'number', 2, 'kWh/unité'),
      createQuestion('hosp_env_2', DiagnosticCategory.ENVIRONMENTAL, 'Gestion de l\'eau (réducteurs, réutilisation, linge à la demande) ?', 'boolean', 2),
      createQuestion('hosp_env_3', DiagnosticCategory.ENVIRONMENTAL, 'Tri/valorisation des biodéchets et huiles ?', 'boolean', 2),
      createQuestion('hosp_env_4', DiagnosticCategory.ENVIRONMENTAL, 'Achats alimentaires durables (local, saison, labels) ?', 'boolean', 2),
      
      createQuestion('hosp_eco_1', DiagnosticCategory.ECONOMIC, 'Taux d\'occupation moyen et durée de séjour ?', 'percentage', 2, '%'),
      createQuestion('hosp_eco_2', DiagnosticCategory.ECONOMIC, 'Part des offres bas-carbone (vélo, train, compensation) ?', 'percentage', 2, '%'),
      createQuestion('hosp_eco_3', DiagnosticCategory.ECONOMIC, 'Réduction du gaspillage alimentaire (%) ?', 'percentage', 2, '%'),
      createQuestion('hosp_eco_4', DiagnosticCategory.ECONOMIC, 'Investissements d\'efficacité (CVC, froid, éclairage) ?', 'number', 1, 'MAD'),
      
      createQuestion('hosp_social_1', DiagnosticCategory.SOCIAL, 'Conditions de travail (horaires, saisonnalité, logement) ?', 'choice', 2, undefined, ['Excellentes', 'Bonnes', 'Moyennes']),
      createQuestion('hosp_social_2', DiagnosticCategory.SOCIAL, 'Formation client/accueil à l\'écoresponsabilité ?', 'boolean', 2),
      createQuestion('hosp_social_3', DiagnosticCategory.SOCIAL, 'Relations avec acteurs locaux (guides, producteurs) ? Accessibilité (PMR) et sécurité sanitaire ?', 'boolean', 2),
      
      createQuestion('hosp_log_1', DiagnosticCategory.LOGISTICS, 'Approvisionnements locaux (km, % fournisseurs) ?', 'percentage', 2, '%'),
      createQuestion('hosp_log_2', DiagnosticCategory.LOGISTICS, 'Livraisons mutualisées et fenêtres horaires optimisées ?', 'boolean', 2),
      createQuestion('hosp_log_3', DiagnosticCategory.LOGISTICS, 'Gestion des retours emballages et bouteilles consignées ?', 'boolean', 2),
      createQuestion('hosp_log_4', DiagnosticCategory.LOGISTICS, 'Mobilité des clients/employés (plans mobilité) ?', 'boolean', 1),
    ],
  },
  ...
  [SectorCategory.REAL_ESTATE]: {
    ...
    questions: [
      createQuestion('real_env_1', DiagnosticCategory.ENVIRONMENTAL, 'Performance énergétique moyenne du parc (kWh/m²·an) ?', 'number', 3, 'kWh/m²·an'),
      createQuestion('real_env_2', DiagnosticCategory.ENVIRONMENTAL, 'Part de rénovations lourdes / BBC réalisées (%) ?', 'percentage', 2, '%'),
      createQuestion('real_env_3', DiagnosticCategory.ENVIRONMENTAL, 'Gestion de l\'eau et végétalisation (îlots de fraîcheur) ? Gestion circulaire des déchets de maintenance ?', 'boolean', 2),
      
      createQuestion('real_eco_1', DiagnosticCategory.ECONOMIC, 'Taux de vacance et rotation des locataires ?', 'percentage', 2, '%'),
      createQuestion('real_eco_2', DiagnosticCategory.ECONOMIC, 'CAPEX vert (isolation, HVAC, PV) sur 3 ans ?', 'number', 2, 'MAD'),
      createQuestion('real_eco_3', DiagnosticCategory.ECONOMIC, 'Montage financier (CEE, subventions, tiers-financement) ? Pilotage GTB / smart meters (optimisation) ?', 'boolean', 2),
      
      createQuestion('real_social_1', DiagnosticCategory.SOCIAL, 'Logements abordables / sociaux (part du parc) ?', 'percentage', 2, '%'),
      createQuestion('real_social_2', DiagnosticCategory.SOCIAL, 'Accessibilité et adaptation (PMR, seniors) ?', 'boolean', 2),
      createQuestion('real_social_3', DiagnosticCategory.SOCIAL, 'Qualité de service (délai d\'intervention, satisfaction) ? Dispositifs anti-précarité énergétique ?', 'choice', 2, undefined, ['Excellente', 'Bonne', 'Moyenne']),
      
      createQuestion('real_log_1', DiagnosticCategory.LOGISTICS, 'Approvisionnements responsables pour maintenance ?', 'boolean', 2),
      createQuestion('real_log_2', DiagnosticCategory.LOGISTICS, 'Boucles d\'emploi des matériaux (réhab) ?', 'boolean', 2),
      createQuestion('real_log_3', DiagnosticCategory.LOGISTICS, 'Plan mobilité des occupants (mobilité douce) ?', 'boolean', 2),
      createQuestion('real_log_4', DiagnosticCategory.LOGISTICS, 'Gestion des prestataires (SLA, traçabilité) ?', 'boolean', 1),
    ],
  },
